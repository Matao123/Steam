		...
		function navType(){
			$(".scheme").hide()
			$(".scheme").removeClass("focus")
			$(".scheme").eq(x).fadeIn()
			$(".scheme").eq(x).addClass("focus")
		}

		$(".highlight_item").click(function(){
			x = $(this).attr("index")
			navType()
		...
		var surname = false
		var count = $(".highlight_item").length
		var x = 0
		function toNext(){
    		if(x == count){
				x = 0
			...
			navType()
     		var t2 = setInterval(function(){
       			isAnimationHas = true
        		// 移动的位移
        		var ml = parseInt($(".highlight_selector")[0].style.left);
        		// 要移动的总距离
       			var far = x*120
        		if(ml !== far){
         			$(".highlight_selector")[0].style.left = ml + Math.ceil((far-ml)/10) + "px"
       			}
       			// 当动画执行完毕后
      			if(ml == far){	
					clearInterval(t2)
					...
					if(x == count){
						// ul瞬间到达真实的第一张
          				$(".highlight_selector")[0].style.left = 0
            			// 将n校准到正确位置
						x = 0
      				}
      			}
			...
			var t3 = setInterval(function(){
	  			...
	  			if(x % 5 == 0 && x != 0 && x != 15){
					...
					isAnimationHas = true
	        		if(ml !== far){
	         			$(".highlight_strip_scroll")[0].style.left = ml + Math.floor((far-ml)/10) + "px"
	       			}
	  				if(ml == far){	
						clearInterval(t3)
						isAnimationHas = false
